...
NSDevilX::NSRenderSystem::NSGL4::CViewportImp::CViewportImp(IViewportImp * interfaceImp)
...
Void NSDevilX::NSRenderSystem::NSGL4::CViewportImp::prepare()
{
	process();
	mTask->prepare();
}
...
Void NSDevilX::NSRenderSystem::NSGL4::CViewportImp::onMessage(CWindowImp * notifier,UInt32 message,VoidPtr data,Bool & needNextProcess)
{
	switch(message)
	{
...
Boolean NSDevilX::NSRenderSystem::NSGL4::CViewportImp::_process(UInt32 flagIndex,OUT Bool & needNextProcess)
{
	switch(flagIndex)
	{
...
Void NSDevilX::NSRenderSystem::NSGL4::CViewportImp::_updateRenderTask()
{
	DEVILX_DELETE(mTask);
	mTask=nullptr;
	switch(getInterfaceImp()->getTechnique())
