				...
				wnd_class.lpfnWndProc=[](HWND wnd,UINT msg,WPARAM wParam,LPARAM lParam)
				{
					...
					if(window)
					{
						...
						case WM_IME_COMPOSITION:
							...
							{
								auto context=ImmGetContext(wnd);

								// Get the size of the result string. 
								auto text_size=ImmGetCompositionString(context,GCS_RESULTSTR,NULL,0);

								auto mem=GlobalAlloc(GHND,text_size);

								auto str=(LPSTR)GlobalLock(mem);

								// Get the result strings that is generated by IME into lpstr. 
								ImmGetCompositionString(context,GCS_RESULTSTR,str,text_size);
								ImmReleaseContext(wnd,context);
								CUTF16String utf16_str(str,text_size);
								// add this string into text buffer of application 
								for(const auto & ch:utf16_str)
								{
						...
						case WM_CHAR:
							for(auto listener:window->mEventListeners)
								listener->onCharEvent(window,CUTF16Char(&wParam,False));
							break;
						case WM_LBUTTONDOWN:
							for(auto listener:window->mEventListeners)
