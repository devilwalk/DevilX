...
Void NSDevilX::NSCubeBlockSystem::ISceneImp::setBlockMT(DirectX::FXMVECTOR positionVec,IBlock * block)
{
	CSInt3 block_position(positionVec);
	DirectX::XMVECTOR area_size_vec=CSInt3(CArea::sBlockSize);
	...
	const DirectX::XMVECTOR area_pos_vec=CSInt3((positionVec-value_offset_vec)/area_size_vec)+value_offset_vec;
	...
	area->setBlockMT(positionVec-area_pos_vec*area_size_vec,static_cast<IBlockImp*>(block));
}
...
IBlock * NSDevilX::NSCubeBlockSystem::ISceneImp::getBlockMT(DirectX::FXMVECTOR positionVec)
{
	CSInt3 block_position(positionVec);
	DirectX::XMVECTOR area_size_vec=CSInt3(CArea::sBlockSize);
	DirectX::XMVECTOR value_offset_vec=CSInt3((block_position.x<0)?-1:0,(block_position.y<0)?-1:0,(block_position.z<0)?-1:0);
	CSInt3 value_offset(value_offset_vec);
	const DirectX::XMVECTOR area_pos_vec=positionVec/area_size_vec+value_offset_vec;
	const DirectX::XMVECTOR area_pos_abs_vec=DirectX::XMVectorAbs(area_pos_vec);
	const CSInt3 area_pos_abs(area_pos_abs_vec);
	...
	if(!area)
		return nullptr;
	return area->getBlockMT(positionVec%area_size_vec);
}
...
{
	const CSInt3 area_pos_abs(absPositionVec);
...
Void NSDevilX::NSCubeBlockSystem::ISceneImp::_setBlockToAreaMT(const CRange3I & range,IBlockImp * block)
{
	...
	const DirectX::XMVECTOR area_size_vec=CSInt3(CArea::sBlockSize);
	const CSInt3 min_mod=DirectX::XMVectorGreaterOrEqual(block_ranges_min_vec,CSInt3::sZero);
	const CSInt3 max_mod=DirectX::XMVectorGreaterOrEqual(block_ranges_max_vec,CSInt3::sZero);
	const CSInt3 min_offset((min_mod.x==0)?-1:0,(min_mod.y==0)?-1:0,(min_mod.z==0)?-1:0);
	const CSInt3 max_offset((max_mod.x==0)?-1:0,(max_mod.y==0)?-1:0,(max_mod.z==0)?-1:0);
	const DirectX::XMVECTOR min_offset_vec=min_offset;
	const DirectX::XMVECTOR max_offset_vec=max_offset;
	const CRange3I area_ranges(CSInt3((block_ranges_min_vec-min_offset)/area_size_vec)+min_offset,CSInt3((block_ranges_max_vec-max_offset)/area_size_vec)+max_offset);
	CRange3I intersection(CSInt3::sZero,CSInt3::sZero);
	for(auto x=area_ranges.getMin().x;x<=area_ranges.getMax().x;++x)
	{
		for(auto y=area_ranges.getMin().y;y<=area_ranges.getMax().y;++y)
		{
			...
			{
				const CSInt3 area_pos(x,y,z);
				const DirectX::XMVECTOR area_pos_vec=area_pos;
				const DirectX::XMVECTOR area_pos_abs_vec=DirectX::XMVectorAbs(area_pos_vec);
				CSInt3 value_offset((area_pos.x<0)?-1:0,(area_pos.y<0)?-1:0,(area_pos.z<0)?-1:0);
				auto & areas_xyz=mAreas[value_offset.x+1][value_offset.y+1][value_offset.z+1];
				CArea * area=_createOrRetrieveAreaMT(areas_xyz,area_pos_abs_vec,area_pos_vec);
				const CRange3I area_range(area->getPosition()*area_size_vec,area->getPosition()*area_size_vec+area_size_vec-CSInt3::sOne);
