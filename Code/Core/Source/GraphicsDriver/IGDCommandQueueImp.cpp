...
NSDevilX::NSCore::NSGraphicsDriver::ICommandQueueImp::ICommandQueueImp(IDeviceImp* dev)
	:mDevice(dev)
{
}

NSDevilX::NSCore::NSGraphicsDriver::ICommandQueueImp::~ICommandQueueImp()
{
}

#if DEVILX_WINDOW_SYSTEM==DEVILX_WINDOW_SYSTEM_WINDOWS

ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::ICommandQueueImp::createSwapChain(DXGI_SWAP_CHAIN_DESC& desc)
{
	...
	return static_cast<ICommandQueue*>(this)->createSwapChain(desc.OutputWindow,desc1,&full_screen_desc);
}

ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::ICommandQueueImp::createSwapChain(DXGI_SWAP_CHAIN_DESC& desc,const DXGI_FORMAT* formats,UInt32 formatCount)
{
...
ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::ICommandQueueImp::createSwapChain(DXGI_SWAP_CHAIN_DESC* descs,UInt32 descsCount)
{
...
ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::ICommandQueueImp::createSwapChain(HWND hwnd,const DXGI_SWAP_CHAIN_DESC1& desc,const DXGI_FORMAT* formats,UInt32 formatCount,const DXGI_SWAP_CHAIN_FULLSCREEN_DESC* fullscreenDesc)
{
	auto ret=static_cast<ICommandQueue*>(this)->createSwapChain(hwnd,desc,fullscreenDesc);
	auto validate_desc=desc;
	UInt32 index=0;
	while(!ret)
	{
		...
		ret=static_cast<ICommandQueue*>(this)->createSwapChain(hwnd,validate_desc,fullscreenDesc);
...
ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::ICommandQueueImp::createSwapChain(HWND hwnd,const DXGI_SWAP_CHAIN_DESC1* descs,UInt32 descCount,const DXGI_SWAP_CHAIN_FULLSCREEN_DESC* fullscreenDesc)
{
	ISwapChain* ret=nullptr;
	UInt32 index=0;
	while(!ret)
	{
		...
		ret=static_cast<ICommandQueue*>(this)->createSwapChain(hwnd,descs[index],fullscreenDesc);
...
ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::ICommandQueueImp::createSwapChain(HWND hwnd,const VkSwapchainCreateInfoKHR& info,const VkFormat* formats,UInt32 formatCount)
{
	auto ret=static_cast<ICommandQueue*>(this)->createSwapChain(hwnd,info);
	...
	while(!ret)
	{
		...
		ret=static_cast<ICommandQueue*>(this)->createSwapChain(hwnd,validate_desc);
...
ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::ICommandQueueImp::createSwapChain(HWND hwnd,const VkSwapchainCreateInfoKHR* infos,UInt32 infoCount)
{
	ISwapChain* ret=nullptr;
	UInt32 index=0;
	while(!ret)
	{
		...
		ret=static_cast<ICommandQueue*>(this)->createSwapChain(hwnd,infos[index]);
...
NSDevilX::NSCore::NSGraphicsDriver::NSD3D11::ICommandQueueImp::ICommandQueueImp(IDeviceImp* dev,ID3D11DeviceContext* context)
	:NSGraphicsDriver::ICommandQueueImp(dev)
	,mInternal(context)
{
}

NSDevilX::NSCore::NSGraphicsDriver::NSD3D11::ICommandQueueImp::~ICommandQueueImp()
{

}

ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::NSD3D11::ICommandQueueImp::createSwapChain(DXGI_SWAP_CHAIN_DESC& desc)
{
	IDXGISwapChain* sc=nullptr;
	auto success=SUCCEEDED(static_cast<NSD3D::IInstanceImp*>(mDevice->getPhysicalDeviceGroup()->getInstance())->getInternal()->CreateSwapChain(mInternal,&desc,&sc));
	if(success)
	{
...
ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::NSD3D11::ICommandQueueImp::createSwapChain(HWND hwnd,const DXGI_SWAP_CHAIN_DESC1& desc,const DXGI_SWAP_CHAIN_FULLSCREEN_DESC* fullscreenDesc)
{
...
ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::NSD3D11::ICommandQueueImp::createSwapChain(HWND hwnd,const VkSwapchainCreateInfoKHR& info)
{
...
NSDevilX::NSCore::NSGraphicsDriver::NSD3D12::ICommandQueueImp::ICommandQueueImp(NSD3D12::IDeviceImp* dev,ID3D12CommandQueue* queue)
	:NSGraphicsDriver::ICommandQueueImp(dev)
	,mInternal(queue)
{
}

NSDevilX::NSCore::NSGraphicsDriver::NSD3D12::ICommandQueueImp::~ICommandQueueImp()
{
}

ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::NSD3D12::ICommandQueueImp::createSwapChain(HWND hwnd,const DXGI_SWAP_CHAIN_DESC1& desc,const DXGI_SWAP_CHAIN_FULLSCREEN_DESC* fullscreenDesc)
{
	IDXGISwapChain1* sc=nullptr;
	auto success=SUCCEEDED(static_cast<NSD3D::IInstanceImp*>(mDevice->getPhysicalDeviceGroup()->getInstance())->getInternal2()->CreateSwapChainForHwnd(mInternal,hwnd,&desc,fullscreenDesc,nullptr,&sc));
	if(success)
...
ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::NSD3D12::ICommandQueueImp::createSwapChain(HWND hwnd,const VkSwapchainCreateInfoKHR& info)
{
	DXGI_SWAP_CHAIN_DESC1 desc={};
	switch(info.compositeAlpha)
	{
...
NSDevilX::NSCore::NSGraphicsDriver::NSVulkan::ICommandQueueImp::ICommandQueueImp(NSVulkan::IDeviceImp* dev,UInt32 familyIndex,UInt32 queueIndex)
	:NSGraphicsDriver::ICommandQueueImp(dev)
	...
	,mQueueIndex(queueIndex)
{
	...
	vkGetDeviceQueue2(dev->getInternal(),&info,&mInternal);
}

NSDevilX::NSCore::NSGraphicsDriver::NSVulkan::ICommandQueueImp::~ICommandQueueImp()
{
}

#if DEVILX_WINDOW_SYSTEM==DEVILX_WINDOW_SYSTEM_WINDOWS
ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::NSVulkan::ICommandQueueImp::createSwapChain(HWND hwnd,const DXGI_SWAP_CHAIN_DESC1& desc,const DXGI_SWAP_CHAIN_FULLSCREEN_DESC* fullscreenDesc)
{
...
ISwapChain* NSDevilX::NSCore::NSGraphicsDriver::NSVulkan::ICommandQueueImp::createSwapChain(HWND hwnd,const VkSwapchainCreateInfoKHR& info)
{
	auto cpy_info=info;
	if(cpy_info.sType==0)
	{
	...
	cpy_info.imageSharingMode=VK_SHARING_MODE_EXCLUSIVE;
	cpy_info.pQueueFamilyIndices=nullptr;
	cpy_info.queueFamilyIndexCount=0;
	RECT rc={};
	GetClientRect(hwnd,&rc);
	...
	if((VK_SUCCESS!=vkGetPhysicalDeviceSurfaceSupportKHR(static_cast<IPhysicalDeviceImp*>(mDevice->getPhysicalDeviceGroup()->getDevices()[0])->getInternal(),mFamilyIndex,cpy_info.surface,&surface_support))
		&&(!surface_support))
	{
	...
	cpy_info.minImageCount=std::min(cpy_info.minImageCount,surface_cap.surfaceCapabilities.maxImageCount);
	...
	if(!cpy_info.compositeAlpha)
	{
		for(int i=0;i<=3;++i)
		{
			auto flag=(VkCompositeAlphaFlagBitsKHR)(1<<i);
			if(flag&surface_cap.surfaceCapabilities.supportedCompositeAlpha)
			{
				cpy_info.compositeAlpha=flag;
				break;
			}
		}
	}
	if(!(cpy_info.compositeAlpha&surface_cap.surfaceCapabilities.supportedCompositeAlpha))
	{
		vkDestroySurfaceKHR(static_cast<IInstanceImp*>(mDevice->getPhysicalDeviceGroup()->getInstance())->getInternal(),cpy_info.surface,nullptr);
		return nullptr;
	}
