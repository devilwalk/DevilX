...
Void NSDevilX::C3DTreeNode::fillChildrenData()
{
	...
	CRange3I ranges[8];
	CInt3 half_extend=(range.getMax()-range.getMin()+CInt3::sOne)/2;
	ranges[0]=CRange3I(range.getMin(),range.getMin()+half_extend-CInt3::sOne);
	...
	ranges[7]=CRange3I(range.getMin()+half_extend-CInt3::sOne,range.getMax());
	if(half_extend==CInt3::sOne)
	{
		for(int i=0;i<8;++i)
		{
			(*static_cast<SData*>(mData)->mChildren)[i]=DEVILX_NEW C3DTreeLeaf(ranges[i].getMin());
		}
	}
	else
	{
		for(int i=0;i<8;++i)
		{
			(*static_cast<SData*>(mData)->mChildren)[i]=DEVILX_NEW C3DTreeNode(ranges[i]);
		}
	}
}
